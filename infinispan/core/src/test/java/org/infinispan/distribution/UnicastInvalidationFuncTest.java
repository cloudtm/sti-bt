...
public class UnicastInvalidationFuncTest extends BaseDistFunctionalTest {
   ...
   public void testPut() {
      ...
      assertNull(nonOwner.getAdvancedCache().getDataContainer().get(KEY1, null));
      assertEquals(owner.getAdvancedCache().getDataContainer().get(KEY1, null).getValue(), "foo");
      
      // Request from another non-owner so that we can get an invalidation command there
      assertEquals(secondNonOwner.get(KEY1), "foo");
      assertEquals(secondNonOwner.getAdvancedCache().getDataContainer().get(KEY1, null).getValue(), "foo");
