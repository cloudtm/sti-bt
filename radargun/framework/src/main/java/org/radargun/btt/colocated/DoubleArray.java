...
public class DoubleArray<T extends Serializable> implements Serializable {
    ...
    public DoubleArray(Class<T> valuesClazz, Comparable key, T value, T lastValue) {
        ...
        this.keys[1] = BPlusTree.LAST_KEY;
    ...
    private <E> int binarySearchForInsertion(E[] array, Comparable key) {
        int result = Arrays.binarySearch(array, key, BPlusTree.COMPARATOR_SUPPORTING_LAST_KEY);
        if (result < 0) {
            result *= -1;
            result--;
        }
    ...
    public T get(Comparable key) {
        int index = Arrays.binarySearch(keys, key, BPlusTree.COMPARATOR_SUPPORTING_LAST_KEY);
        if (index >= 0) {
            return values[index];
        } else {
            return null;
    ...
    public String toString() {
        return Arrays.toString(this.keys) + " | " + Arrays.toString(this.values);
    }
    ...
    public DoubleArray<T> replaceKey(int indexToReplace, Comparable replacementKey) {
	Comparable[] newKeys = new Comparable[keys.length];
	T[] newValues = values;
	
	System.arraycopy(keys, 0, newKeys, 0, newKeys.length);
	newKeys[indexToReplace] = replacementKey;
    ...
    public DoubleArray<T> removeKey(Comparable key) {
        ...
        int indexToRemove = Arrays.binarySearch(keys, key, BPlusTree.COMPARATOR_SUPPORTING_LAST_KEY);
    ...
    public Comparable findRightMiddlePosition() {
        return keys[BPlusTree.LOWER_BOUND + 1];
    }
    ...
    public DoubleArray<T> changeReplication(T[] newValues) {
	Comparable[] clonedKeys = new Comparable[keys.length];
	System.arraycopy(keys, 0, clonedKeys, 0, keys.length);
	
	return new DoubleArray<T>(valuesClazz, clonedKeys, newValues);
    }
    
    public DoubleArray<T> replaceChild(T oldElem, T newElem) {
	Comparable[] clonedKeys = new Comparable[keys.length];
	System.arraycopy(keys, 0, clonedKeys, 0, keys.length);
	T[] clonedValues = (T[]) Array.newInstance(valuesClazz, values.length);
	System.arraycopy(values, 0, clonedValues, 0, values.length);
	
	for (int i = 0; i < clonedValues.length; i++) {
	    if (clonedValues[i].equals(oldElem)) {
		clonedValues[i] = newElem;
		return new DoubleArray<T>(valuesClazz, clonedKeys, clonedValues);
	    }
	}
	
	System.err.println("Could not find old inner node to replace child");
	System.exit(-1);
